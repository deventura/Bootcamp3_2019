...
*/
  ...
  //my test case to update a listing with coordinates, also makes sure that coordinates are still saved for the listing
  it('my test case: should be able to update a listing with coordinates', function(done) {
    var updatedListing2 = {
      code: 'GMC',
      name: 'Dr. Christina Gardner-McCunes Office',
      address: '432 Newell Dr, Gainesville, FL 32611'
    };

    agent.put('/api/listings/' + id2)
      .send(updatedListing2)
      .expect(200)
      .end(function(err, res) {
        should.not.exist(err);
        should.exist(res.body._id);
        res.body.name.should.equal('Dr. Christina Gardner-McCunes Office');
        res.body.code.should.equal('GMC');
        res.body.address.should.equal('432 Newell Dr, Gainesville, FL 32611');
        res.body.coordinates.latitude.should.equal(28.75054);
        res.body.coordinates.longitude.should.equal(-82.5001);
